...
$teatreEvents = array_filter($events, function ($event) {
    return isset($event['type']) && $event['type'] === 'teatre';
});
$title = "Events de Teatre - Festival de Música 2024";
...
<body>
    ...
    <main>
        <section id="events">
            <h2>Events Afegits de Teatre</h2>
            <?php if (!empty($teatreEvents)): ?>
                <ul>
                    <?php foreach ($teatreEvents as $event): ?>
                        <li>
            ...
            <?php else: ?>
                <p>No s'han afegit events de teatre encara.</p>
            <?php endif; ?>
        </section>
